...
const CONFIG = {
  ...
  privatePrompts: [
    ...
    { id:'prompt-3', title:'通用万能优化', desc:'用 LangGPT 结构化框架优化你的简短指令，适配 GPT', tags:['提示词优化','LangGPT','GPT'], content:'请使用 LangGPT 结构化框架优化下面这条提示词，补充角色、约束、工作流程、输出格式；不要编造额外需求，保留我的原始目标，输出一份可以直接粘贴进 GPT 使用的完整提示词：' },
    { id:'prompt-4', title:'多步骤自动化Agent', desc:'生成指定场景的自动化 Agent 提示词，全自动执行', tags:['Agent','LangGPT','自动化','提示词生成'], content:'帮我用 LangGPT 结构化格式，生成一个【xxx场景】专属多步骤自动化Agent提示词，包含：角色、能力、执行规则、分步工作流、标准化输出格式，做到全自动执行、无需人工干预、可直接复制使用。' }
  ],
  externalPrompts: [
    /* ===== 国内直连 · 中文优先（中文综合平台） ===== */
